local hsluv = require("cobalt.lib.hsluv")
...
function Color.new(hex)
    ...
    local H, S, L = table.unpack(hsluv.hex_to_hsluv(hex))
...
local function blendRGB(a, b, r)
    local c = {}
    for i = 1, 3 do
        c[i] = math.sqrt((1 - r) * a[i]^2 + r * b[i]^2)
    end
    return c
end
...
function Color:blend(b, r)
    ...
    self.H, self.S, self.L = table.unpack(hsluv.rgb_to_hsluv(c))
    return self
end
